...
BlockController.prototype.getBlockByHash = function (hash, next) {
  ...
  if (blockCached) {
    ...
    return async.waterfall([ function (callback) {
      ...
      var blockResult = self.transformBlock(dataFlow.block, dataFlow.info, dataFlow.reward);
...
BlockController.prototype.transformBlock = function(block, info, reward) {
  ...
  return {
    ...
    reward: reward / 1e8,
    isMainChain: (blockHeader.confirmations !== -1),
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    return self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }
